...
const apis: MockMethod[] = [
  // 获取验证码
  {
    url: '/mock/getSmsCode',
    method: 'post',
    response: (): App.Service.MockServiceResult<boolean> => {
      return {
        code: 200,
        message: 'ok',
        data: true
  ...
  // 用户+密码 登录
  {
    url: '/mock/login',
    method: 'post',
    response: (options: App.Service.MockOption): App.Service.MockServiceResult<Api.Auth.LoginToken | null> => {
  ...
  // 获取用户信息(请求头携带token, 根据token获取用户信息)
  {
    ...
    method: 'get',
    response: (options: App.Service.MockOption): App.Service.MockServiceResult<Api.Auth.UserInfo | null> => {
      ...
      const isInUser = userModel.some(item => {
        ...
        if (flag) {
          const { id: itemUserId, userName, authority } = item;
          Object.assign(userInfo, { userId: itemUserId, userName, authority });
        }
        return flag;
      });
