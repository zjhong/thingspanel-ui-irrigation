...
export default {
  cardItemBase: {
    type: 'plugins', // 卡片类型 'system' | 'plugins' | 'chart';
    id: 'plugins-demo1', // 卡片唯一标识，按照card_type_cardName命名不会错
    sourceNumber: 3, // 数据个数，必须限制，开发卡片时想好
    basicSettings: {
      defaultTitle: 'plugins-demo1', // 卡片标题 尽量使用国际化标题
      showTitle: false
    }, // 初始标题 可以不定义

    scene: 'all', // 'mobile' | 'pc' | 'all'; 预留暂时都选all
    minWH: {
      minW: '200px', // 卡片最小宽度,字符串单位为像素，数字则表示占几个格子，当前默认共48格
      minH: '200px' // 卡片最小高度,字符串单位为像素，数字则表示占几个格子，当前默认一格高度30px
    },
