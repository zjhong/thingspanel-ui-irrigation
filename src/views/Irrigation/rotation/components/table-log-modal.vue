...
import { irrigationRotationHistorys } from '@/service/api/irrigation';
...
interface QueryFormModel {
  rotation_irrigation_id: string;
  page: number;
  page_size: number;
}

const queryParams = reactive<QueryFormModel>({
  rotation_irrigation_id: '',
  page: 1,
  page_size: 10
});
...
async function getTableData() {
  startLoading();
  queryParams.rotation_irrigation_id = props.editData?.id || '';
  const { data } = await irrigationRotationHistorys(queryParams);
  endLoading();
  if (data) {
