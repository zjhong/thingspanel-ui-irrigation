...
const fromRules: Rules = {
  data_name: {
    required: true,
    trigger: ['blur', 'input'],
    message: $t('device_template.table_header.parameterName')
  },
  data_identifier: {
    required: true,
    trigger: ['blur', 'input'],
...
<template>
  <n-form :model="addFrom" :rules="fromRules" label-placement="left" label-width="100" ref="formRef"
    ...
    <n-form-item :label="$t('device_template.table_header.commandName')" path="data_name">
      <n-grid :cols="2">
        <n-gi>
          <n-input v-model:value.trim="addFrom.data_name"
            :placeholder="$t('device_template.table_header.singleControlTask')" />
        </n-gi>
      </n-grid>
    </n-form-item>
    ...
    <div class="box">
      <n-button @click="addParams" class="box-btn">
        ...
        </template>{{ $t('device_template.table_header.addParameters') }}
      </n-button>
    ...
    <n-form-item :label="$t('device_template.table_header.commandDescription')">
      <n-input v-model:value.trim="addFrom.description"
        :placeholder="$t('device_template.table_header.pleaseEnterACommandDescription')" type="textarea" />
    </n-form-item>
  </n-form>
  <div class="box2">
  ...
  <NModal v-model:show="addParameter" preset="card" :title="$t('device_template.table_header.addEditParameters')"
    class="w-30%" @afterLeave="addParameterClone">
    <n-form :model="addParameterFrom" :rules="addParameterRules" label-placement="left" label-width="100" ref="formRefs"
      ...
      <n-form-item :label="$t('device_template.table_header.parameterName')" path="data_name">
        <n-input v-model:value.trim="addParameterFrom.data_name"
      ...
      <n-form-item :label="$t('device_template.table_header.ParameterIdentifier')" path="data_identifier">
        <n-input v-model:value.trim="addParameterFrom.data_identifier"
      ...
      <n-form-item :label="$t('device_template.table_header.ParameterType')" path="read_write_flag">
        <n-select v-model:value="addParameterFrom.read_write_flag" :options="generalOptions"
      ...
      <n-form-item :label="$t('device_template.table_header.description')">
        <n-input v-model:value.trim="addParameterFrom.description"
          :placeholder="$t('device_template.table_header.PleaseEnterADescription')" type="textarea" />
      </n-form-item>
    </n-form>
    <div class="box2">
