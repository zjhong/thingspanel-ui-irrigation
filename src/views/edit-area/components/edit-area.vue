...
import { areaData, spacesData, editArea } from "@/service/api/equipment-map";
...
export default {
  ...
  setup(props, context) {
      ...
      edit() {
        areaData(state.parameterId).then((e) => {
          ...
          if (e.data) {
            ...
            // eslint-disable-next-line no-eval
            state.areaForm.scope = JSON.stringify(e.data.scope);
            console.log("查询详情11111", state.spaces);
            state.spaces.map((item) => {
              console.log(787878, item);
    ...
    onMounted(() => {
      ...
      methods.spacesList();
      methods.edit();
      console.log(1111111111, props.dataId);
    });
