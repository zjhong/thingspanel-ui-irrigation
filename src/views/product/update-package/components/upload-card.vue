<template>
	<n-upload :action="url + '/file/up'" :headers="{
		'x-token': localStg.get('token') || ''
	}" :data="{ type: 'image' }" :default-file-list="dataList" :accept="accept" :max="1" @before-upload="beforeUpload"
		@finish="handleFinish" @error="handleError">
		<n-button>{{ $t('page.product.update-package.package') }}</n-button>
	</n-upload>
</template>
...
import { generateUUID, getFileName, localStg } from '@/utils'
...
const dataList = computed((): UploadFileInfo[] => {
	...
	return props.value?.split(',').map((item: string) => ({
		id: generateUUID(),
